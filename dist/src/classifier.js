...
function countSignalOverlap(content, replyText) {
    ...
    for (const m of content.toLowerCase().match(/[a-z][a-z0-9]{1,}/g) ?? [])
        tokens.add(m);
...
async function classifyWithLLM(content, activeTopic, allTopics, recentMessages, llmConfig, log, lastAssistantReply, freshAutoNewParent) {
...
// ---------------------------------------------------------------------------
export async function classify(content, recentMessages, registry, config, llmConfig, log, lastAssistantReply, freshAutoNewParent) {
    const noop = () => { };
    const _log = log ?? noop;
    ...
    if (useHybrid && llmConfig?.apiKey) {
        if (isLLMCircuitOpen()) {
            ...
            const llmResult = await classifyWithLLM(content, activeTopic, allTopics, recentMessages, llmConfig, _log, lastAssistantReply, freshAutoNewParent);
            if (llmResult)
                return llmResult;
        }
    }
