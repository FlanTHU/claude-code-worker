 ...
 */
function countSignalOverlap(content: string, replyText: string): number {
  if (!content || !replyText) return 0;
...
export function detectShortFollowUp(
  content: string,
  activeTopic: TopicEntry | null,
  recentlyActive: boolean
): ClassifyResult | null {
  if (!activeTopic || !recentlyActive) return null;
...
export async function classify(
  ...
  if (expired > 0) _log(`[classifier] Auto-ended ${expired} stale inactive topic(s) (idle > ${expireHours}h)`);
